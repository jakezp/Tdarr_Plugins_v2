    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            ...
            case 6:
                ...
                streamsToConvert = audioStreams.filter(function (stream) {
                ...
                args.jobLog("Found ".concat(streamsToConvert.length, " audio streams to convert"));
                fileDir = path.dirname(filePath);
                fileName = path.basename(filePath, path.extname(filePath));
                fileExt = path.extname(filePath);
                outputFilePath = "".concat(fileDir, "/").concat(fileName, "_ac3").concat(fileExt);
                ffmpegArgs_1 = [
                    '-i', filePath,
                    '-map', '0',
                    '-c:v', 'copy',
                ...
                audioStreams.forEach(function (stream, index) {
                    ...
                    var needsConversion = stream.codec_name !== 'ac3' || stream.channels > 6;
                    if (needsConversion) {
                        ...
                        ffmpegArgs_1.push("-ac:a:".concat(index), outputChannels.toString());
                        // Set language if available
                        var lang = ((_a = stream.tags) === null || _a === void 0 ? void 0 : _a.language) || defaultLanguage_1;
                        ffmpegArgs_1.push("-metadata:s:a:".concat(index), "language=".concat(lang));
                        // Copy any other metadata
                        if ((_b = stream.tags) === null || _b === void 0 ? void 0 : _b.title) {
                            ffmpegArgs_1.push("-metadata:s:a:".concat(index), "title=".concat(stream.tags.title));
                        }
                    }
                    else if (preserveOriginal_1) {
                        // Stream is already AC3 with 6 or fewer channels, just copy it
                        args.jobLog("Stream ".concat(streamIndex, " is already AC3 with ").concat(stream.channels, " channels, copying"));
                        ffmpegArgs_1.push("-c:a:".concat(index), 'copy');
                    }
                    else {
                        // Stream is already AC3 with 6 or fewer channels, just copy it
                        args.jobLog("Stream ".concat(streamIndex, " is already AC3 with ").concat(stream.channels, " channels, copying"));
                ...
                // Add output file
                ffmpegArgs_1.push(outputFilePath);
                args.jobLog("Executing FFmpeg command to convert audio streams");
                args.jobLog("FFmpeg arguments: ".concat(ffmpegArgs_1.join(' ')));
                cli = new cliUtils_1.CLI({
                    cli: args.ffmpegPath,
                    spawnArgs: ffmpegArgs_1,
                    spawnOpts: {},
                    jobLog: args.jobLog,
